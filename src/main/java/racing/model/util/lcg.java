...
public class LCG {
    ...
    public LCG() {
        ...
        this.seed = (int) (new Date().getTime() / 1000);
        this.modulus = (1 << 16) + 1;
        this.multiplier = 75;
        this.increment = 0;
    }
    ...
    public void lcg(int[] randomNumbers, int numberOfRandomNumbers) {
        randomNumbers[0] = this.seed;

        for (int i = 1; i < numberOfRandomNumbers; i++) {
            randomNumbers[i] =
                ((randomNumbers[i - 1] * this.multiplier) + this.increment) %
                this.modulus;
        }
    }

    public int[] randomNumbers(int[] arr, int n) {
        lcg(arr, n);
        return arr;
    }
